...
(ns ^:no-doc inline.potemkin.collections
  (:use
    [inline.potemkin types macros utils]))
...
(def-abstract-type AbstractMap
  ...
  inline.potemkin.collections.PotemkinMap
  ...
  clojure.lang.IObj
  (withMeta [this mta]
    (inline.potemkin.collections/with-meta* this mta))
  (meta [this]
    (inline.potemkin.collections/meta* this))
  (meta* [this]
    nil)
  ...
  (count [this]
    (count (inline.potemkin.collections/keys* this)))

  clojure.lang.Seqable
  (seq [this]
    (seq
      (map
        #(inline.potemkin.PersistentMapProxy$MapEntry. this %)
        (inline.potemkin.collections/keys* this))))
  ...
  (hasheq [this]
    (inline.potemkin.collections/compile-if (resolve 'clojure.core/hash-unordered-coll)
  ...
  (valAt [this k default]
    (inline.potemkin.collections/get* this k default))
  ...
  (entryAt [this k]
    (when (contains? (.keySet this) k)
      (inline.potemkin.PersistentMapProxy$MapEntry. this k)))

  (assoc [this k v]
    (inline.potemkin.collections/assoc* this k v))

  (empty* [this]
    {})

  (empty [this]
    (inline.potemkin.collections/empty* this))
  ...
  (size [this]
    (count this))
  (keySet [this]
    (set (inline.potemkin.collections/keys* this)))
  (put [_ _ _]
  ...
  (assocEx [this k v]
    (if (contains? this k)
      ...
      (assoc this k v)))
  (without [this k]
    (inline.potemkin.collections/dissoc* this k))

  inline.potemkin.collections/PotemkinFn

  (invoke [this k]
    (inline.potemkin.collections/get* this k nil))
...
(defmacro def-map-type
  ...
  (let [fns '{get get*
    ...
    (unify-gensyms
      `(do
         (deftype+ ~name ~params ~'inline.potemkin.collections/AbstractMap
           ~@(map
               #(if (sequential? %)
...
(defmacro reify-map-type
  ...
  (let [fns '{get get*
        ...
        elide? '#{withMeta meta}]
    (->>
      `(reify+ ~'inline.potemkin.collections/AbstractMap
         ~@(map
             #(if (sequential? %)
                (list* (get fns (first %) (first %)) (rest %))
                %)
